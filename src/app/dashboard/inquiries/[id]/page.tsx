...
import { ArrowLeft, Calendar, DollarSign, Mail, MapPin, MessageSquare, Phone, Users, Map } from "lucide-react"
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <Link href="/dashboard/inquiries" className="inline-flex items-center justify-center rounded-lg border border-border bg-background hover:bg-muted hover:text-foreground h-10 w-10 shrink-0 shadow-sm">
          <ArrowLeft className="h-5 w-5" />
        </Link>
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-3xl font-bold tracking-tight">Inquiry Details</h1>
            <Badge className="bg-blue-500/10 text-blue-500 hover:bg-blue-500/20 px-3 py-1 text-xs font-semibold">{inquiry.status}</Badge>
          </div>
          <p className="text-muted-foreground text-sm mt-1">Received on {inquiry.createdAt.toLocaleDateString()}</p>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
          ...
          <CardContent className="space-y-6 pt-6">
            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
              ...
              <div className="space-y-1.5 p-3 bg-muted/20 rounded-lg border border-border/30">
                ...
                <p className="font-medium text-lg">{inquiry.eventType}</p>
              </div>
              <div className="space-y-1.5 p-3 bg-muted/20 rounded-lg border border-border/30">
                ...
                <p className="font-medium text-lg">{inquiry.guestCount || "Not specified"}</p>
              </div>
              <div className="space-y-1.5 p-3 bg-muted/20 rounded-lg border border-border/30">
        ...
        <div className="space-y-6">
          <AIInsightsCard inquiryText={inquiry.message} />
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-4 pt-1">
                <div className="flex items-center gap-3 text-sm">
                  ...
                  <a href={`mailto:${inquiry.customerEmail}`} className="hover:underline font-medium">{inquiry.customerEmail}</a>
                </div>
                {inquiry.customerPhone && (
                  <div className="flex items-center gap-3 text-sm">
                    ...
                    <a href={`tel:${inquiry.customerPhone}`} className="hover:underline font-medium">{inquiry.customerPhone}</a>
                  </div>
                )}
              </div>
            </CardContent>
                ...
                <div className="relative flex items-center gap-4">
                  ...
                  <div className="flex-1 p-3 rounded-lg border border-border/50 shadow-sm bg-muted/20">
                    <div className="flex items-center justify-between space-x-2 mb-1">
                      ...
                      <time className="text-xs font-medium text-muted-foreground">{inquiry.createdAt.toLocaleDateString()}</time>
                    </div>
